        ...
        <div className="flex items-center gap-1 sm:gap-2">
          {user ? (
            <div className="hidden md:flex items-center gap-2">
              {isAdmin && (
                <Link to="/admin"
                  className="badge-retail hover:bg-botanica-200 dark:hover:bg-botanica-700 transition-colors cursor-pointer whitespace-nowrap">
                  Administrar
                </Link>
              )}
              <button
                onClick={logout}
                title="Cerrar sesión"
                className="theme-toggle text-botanica-500 dark:text-botanica-400 hover:text-red-500 dark:hover:text-red-400"
          ...
          <span
            title={syncMode === 'supabase' ? 'Sincronizado con Supabase (cross-device)' : 'Datos guardados localmente (solo este dispositivo)'}
            className="hidden sm:flex items-center gap-1.5 text-[10px] font-mono px-2 py-1 rounded-full border transition-colors cursor-default"
            style={{
